...
parser = argparse.ArgumentParser(
    description=""" cube2xyz can convert Gaussian type Cube files into "x y z value" columns, or project values on planes (slizes) or segments. Optionally, it can directly produce the plot for a faster visualization. \n """,
...
parser.add_argument(
    "-o",
    "--out_file",
    help="Name of the file to which print the output (otherwise printed to Standard Output).",
    required=False,
)
parser.add_argument(
    "-pr",
    "--print_range",
    help="Print the range of x,y or z values and exit.",
    required=False,
)
parser.add_argument(
    "-x", "--x_coord", help="Show only the values of x at this point.", required=False
)
parser.add_argument(
    "-y", "--y_coord", help="Show only the values of y at this point.", required=False
)
parser.add_argument(
    "-z", "--z_coord", help="Show only the values of z at this point.", required=False
)
parser.add_argument(
    "-pl",
    "--plot",
    help="Plot graph using Mathplotlib.",
    required=False,
    action="store_true",
)
parser.add_argument(
    "-A",
    "--angstrom",
    help="Convert all distances to Angstrom. (def. a.u.)",
    required=False,
    action="store_true",
)
parser.add_argument(
    "-no",
    "--no_output",
    help="Do not produce any outupt.",
    required=False,
    action="store_true",
)
...
if args["x_coord"]:
    filter = (
        filter
        + " x > "
        + str(float(args["x_coord"]) - float(spacing_vec[0][1]) + 0.1)
        + " and x < "
        + str(float(args["x_coord"]) + float(spacing_vec[0][1]) - 0.1)
        + " "
    )
if args["y_coord"]:
    ...
    filter = (
        filter
        + " y > "
        + str(float(args["y_coord"]) - float(spacing_vec[1][2]) + 0.1)
        + " and y < "
        + str(float(args["y_coord"]) + float(spacing_vec[0][1]) - 0.1)
        + " "
    )
if args["z_coord"]:
    if len(filter) > 3:
    ...
    filter = (
        filter
        + " z > "
        + str(float(args["z_coord"]) - float(spacing_vec[2][3]) + 0.1)
        + " and z < "
        + str(float(args["z_coord"]) + float(spacing_vec[0][1]) - 0.1)
        + " "
    )
if filter == "":
    filter = "1==1"
...
data = []
if (
    args["out_file"] and not args["no_output"]
...
for i in range(0, int(spacing_vec[0][0])):
    for j in range(0, int(spacing_vec[1][0])):
        for k in range(0, int(spacing_vec[2][0])):
            idx += 1
            x, y, z = (
                i * float(spacing_vec[0][1]),
                j * float(spacing_vec[1][2]),
                k * float(spacing_vec[2][3]),
            )
            if eval(filter):
...
if args["plot"]:
    ...
    if plot_dim == 3:
        ...
        X, Y = (
            list(zip(*data))[axe_labels.index(var_axe1)],
            list(zip(*data))[axe_labels.index(var_axe2)],
        )  # yk - 20240409
        
        Z = list(zip(*data))[3]  # yk - 20240409

        # print "max Z: ", max(Z)
        fig = plt.figure()
        ax = plt.subplot(111, projection='3d')
        #  plt.hold(True) # yk - 20240409
        ...
        xc = list(zip(*coord))[axe_labels.index(var_axe1)]  # yk - 20240409
        yc = list(zip(*coord))[axe_labels.index(var_axe2)]  # yk - 20240409
        zc = [max(Z) + (max(Z) * 0.2)] * len(coord)  
        # put the atoms slightly higher than the max value
    ...
    if plot_dim == 2:  # when 2 variables are fixed a 2D plot is produced
        ...
        plot(list(zip(*data))[var_idx], list(zip(*data))[3]) # yk - 20240409
